...
const post: BlogPost = {
  ...
  body: `When we watch someone set up VallaPOS for a food truck for the first time, the whole thing takes about fifteen minutes — and most of that is deciding on menu prices, not fighting the software. Here's where those minutes actually go.
...
This is the part that actually takes time, because it's *your* menu. Add your items and prices — whatever you sell out the window. A few tips that save real time:
...
- **Cash** — on by default; bring enough small bills to make change through the lunch rush.
- **QR** — turn it on in Settings and point it at a payment handle you already have (Venmo, PayPal, Cash App, PIX, UPI, or a payment link). At the window it shows as a code with the order total next to it; the customer scans, taps in the amount, pays, and you confirm before handing over the food.
...
2. **Ring up one item and run a practice sale** as cash, so you've seen the whole flow once before a real customer does.
...
By now the register's real: menu in, payments on, crew's PINs set, drawer open. At the end of the night you close the drawer and the app reconciles your counted cash against what you rang, so night one ends knowing exactly whether you're over, short, or square.

Fifteen minutes, on a phone you already own, with no per-sale cut taken out of your window. A receipt printer and deeper reports are there when you want them — none of it stands between you and selling lunch today.`,
};
