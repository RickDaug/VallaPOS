...
For most small vendors, cash should stay a first-class tender, not a fallback. It's your guaranteed way to get paid when a signal dies or a card reader sulks. Keep a float, and count it against your register total at close so a mistake surfaces the same day.
...
- **Downside:** It costs a percentage of every sale — usually somewhere around 2.6–2.9% plus a fixed fee — and it needs a working connection and a reader. It's the tender most likely to fail in a dead zone.
...
- **Downside:** It leans on the customer having the right app and a signal. And unless you're using a processor-backed setup, it's *confirm-based* — you're checking that the payment actually landed, the same as you would today.
...
- **QR** — shows *your* payment handle as a scannable code at the order's amount; the customer scans and pays, and you confirm receipt.
...
You don't have to accept everything on day one. Start with cash and QR — that combination is free or nearly free and covers most customers — and add card processing when the numbers say it's worth it. Let the register keep all three honest in one place.`,
};
