...
_Last updated: 2026-07-13 — see the **"2026-07-13 — Editions Stage 3 (offline SqliteDataStore complete)"** section just below for the newest work. Older history follows unchanged._
...
## 2026-07-13 — Editions Stage 3 (offline SqliteDataStore complete; #119/#120/#121)
...
**Known deferred in the store (wired later):** operator ATTRIBUTION (`Order.cashierId` / drawer `openedById`) is left null pending the local shell auth boundary. **Next: Stage 4** (local auth + env branch — MODIFIES `env.ts`/`tenant.ts`/`operator.ts` gated on `isLocal`; first cloud-touching editions stage, wants careful verification), then Stage 5 (Tauri shell + native printer + `index.ts` `isLocal` wiring + caller rewire), 6 (license gate + vallahub issuance), 7 (packaging/signing).
