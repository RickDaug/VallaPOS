...
_Last updated: 2026-07-19 — newest: **Editions Stage 5b (partial) — the local-store runtime singleton `initLocalStore`/`getLocalStore` (#138)**; see the **"2026-07-13 — Editions Stages 3–7"** section (offline edition data+auth+Tauri-seam+license+release; #119–#126, #138; the whole TS/build seam is done + tested, cloud unchanged; what's LEFT needs the Rust/Tauri toolchain + signing certs — the turnkey steps are in `docs/SHIP_DESKTOP.md`). Also newest: **"2026-07-19 — Payments PR-D: flat SaaS subscription"** below. Older history follows unchanged._
...
**812→968 tests green across the whole editions arc (incl. #138's +4); cloud byte-for-byte unchanged throughout.**
...
- **Stage 5b (runtime glue ✅ #138; the rest needs a machine):** add the `@tauri-apps` JS deps (+lockfile) + `@tauri-apps/cli` + `npx tauri icon`; convert cash-path `page.tsx` shells to client-fetch through `getLocalStore()` (gated `isLocal`); wire register→auto-print; `DevicesManager` transport swap; inject the real `@tauri-apps` `Database`/`invoke`; then `cargo build` / `tauri build` and verify with `npm run build:local`.
- **Stage 6b-shell** (6b-core TS gate/issuance/store ✅ #126): boot-gate SQLite open on Rust `check_license`; license entry screen (calls `resolveLicenseState`/`createLicenseStore`) + `$APPCONFIG/license.vlk`; `app/(app)/layout.tsx` local license gate; **vallahub.com issuance** (separate site: `LICENSE_SIGNING_SK`→`importEd25519PrivateKey`→shared `issueLicense`, one-time Stripe Checkout webhook, `License` DB row, Resend delivery); replace the license `PUBLIC_KEY`.
