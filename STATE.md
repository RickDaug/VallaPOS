...
**Suite: 382 tests green on `main`** (279 → +103; latest: #72 +5, #74 +15; #73 added the Playwright E2E harness — runs separately via `npm run test:e2e`, not in the 382). CI `quality` still can't run (Actions billing suspended); Vercel deploys ran on each merge.
