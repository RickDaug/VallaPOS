...
- Lower: R-5 CSP still report-only + no report-uri; R-7 offline PII unencrypted at rest (accept/document); R-9 tenant guard is heuristic; R-10 no non-negativity on catalog prices; R-11 `cashierId` not persisted.
...
## Next step
**(1) Fix GitHub Actions billing** so CI runs again (red `quality` everywhere until then). **(2) Merge/sign-off PR #45** (R-1 SW cache) after a human offline click-through. **(3) Activate Upstash** (set valid env vars in Vercel + `.env.local`) to turn on shared rate limiting — no longer blocks deploys (#46), just enhances. **(4) Human browser sign-off** — the one thing automation can't cover: the full ring-up flow (`! npm run db:seed` → sign in → ring up → receipt → refund/void → drawer → employees/clock-in → offline). **(5) Then** the remaining re-audit follow-ups (R-5 CSP enforce w/ nonces, R-10 non-negative catalog prices, R-11 persist `cashierId`, R-7 offline PII at rest, M-3 email verify w/ Resend) and Phase 2/3 candidates: more register UX (Radix Sheet/Numpad, sticky-cart split, image tiles, favorites, open-tickets), multi-sensory tap confirmation, and Phase 3 integrated payments (the monetization milestone).
