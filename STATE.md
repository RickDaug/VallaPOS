...
- **#125 Stage 7a (this PR)** — `.github/workflows/desktop-release.yml` (tauri-action matrix win+mac, `v*`-tag/dispatch only, never gates PRs) + `docs/RELEASING.md` (signing matrix/costs, cut-a-release steps, no-phone-home revocation).

**851→864 tests green across 5a/6a; cloud byte-for-byte unchanged throughout.**
...
- **Stage 6b:** gate SQLite open on `check_license`; license entry screen + `$APPCONFIG/license.vlk`; `app/(app)/layout.tsx` local license gate; **vallahub.com issuance** (separate site: `LICENSE_SIGNING_SK`→`importEd25519PrivateKey`→`signLicense`, one-time Stripe Checkout webhook, `License` DB row, Resend delivery); replace the license `PUBLIC_KEY`.
