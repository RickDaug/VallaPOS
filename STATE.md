...
## 2026-08-09 — 5-perspective QA audit + the fixes that don't need secrets (branch `fix/audit-launch-blockers`)

Ran the 5-lens audit (Principal Eng / QA Lead / POS domain / UX / casual user) against `main` **and against production**, as a pre-launch MVP whose goal is *collect the first dollar*. Verdict: **62/100, DON'T SHIP** — not because the code is weak (it isn't) but because the **deployed configuration cannot take money**. Full findings in the audit conversation; the register below is what this branch does and does not fix.

**The finding that governs everything (⚠ ONLY YOU CAN FIX — needs the real secrets):**
- **`STRIPE_SECRET_KEY` in Vercel Production is malformed.** `env.ts`'s shape check rejects it and drops it to `undefined`. Production runtime log, verbatim: `⚠ SECURITY: STRIPE_SECRET_KEY is set but does not look like a Stripe secret key (expected sk_test_… / sk_live_… / rk_…) — ignoring it.` Consequence: `isDesktopLicenseConfigured()` **and** `isBillingConfigured()` are both false, so **the $99 checkout can't start and the $19.99/mo Subscribe UI never renders**. The Stripe side is fine — live prices `price_1U1ew4AOkezTuccQOSJQUMIb` ($99) and `price_1TyOl2AOkezTuccQD6yRlago` ($19.99/mo) are active and all three webhook endpoints are enabled against `vallapos.com`. Repaste the key (no quotes, no trailing newline), then confirm with `curl https://vallapos.com/api/health`.
- **`RESEND_API_KEY` unset in Prod + Preview** ⇒ **password reset silently does nothing** while still telling the user "Check your inbox" — a merchant who forgets their password is locked out permanently. Also kills email verification + receipt email. (This is launch-checklist item #4; it is a launch blocker, not a nice-to-have.)
- **`LICENSE_SIGNING_SK` + `DESKTOP_DOWNLOAD_URL` unset** ⇒ a paid $99 buyer hits a 503 at `/api/desktop-license/webhook` and receives nothing. **Fix these three TOGETHER or not at all** — repairing Stripe alone opens a checkout that can charge but can't fulfil.

**Shipped on this branch (code only — 1038 tests green, typecheck + lint + build clean):**
- **Marketing routing rewritten from the hash-router artifact to REAL routes** (`scripts/marketing/generate.mjs`). The ported design artifact was a hash-routed SPA, so every internal link shipped as `#/…`: `/about` **404'd** while the nav pointed at `#/about`, every legal policy was reachable only by running the hash router, and all six conversion CTAs were inert `#/#pricing` fragments patched in later by a `useEffect` — so a tap before hydration did nothing. Now `#/x → /x`, `#/#pricing → /#pricing`, and every Buy/Subscribe CTA carries its real destination (`/sign-up`, `/desktop/buy`) **in the server html**. `MarketingSite.tsx` only overrides them when a static Stripe Payment Link env is set.
- **NEW `/about`** — real route rendering `MarketingSite view="about"`; new pure `marketing/view.ts` (`htmlForView`) makes the correct `<main class="view">` visible **server-side** so it isn't a home-page flash. (Pure logic split into `.ts` because vitest can't import `.tsx` under `jsx: "preserve"`.)
- **NEW `src/features/marketing/links.test.ts` (+10)** — the missing property test: no leftover `#/` links, every internal href resolves to a route that exists on disk (route groups included), every same-page anchor resolves to a real id, every `data-buy` CTA points where it should, legal docs stay extractable, no unfilled `[placeholder]`s. 1024 tests and **zero** covered the marketing site; this is why it shipped broken.
- **`/desktop/buy` fails honestly** — new `/desktop/unavailable` page (says what's wrong, offers `hello@vallapos.com` and the cloud edition) instead of a silent 303 back to `/#pricing`. The old fallback was justified as "nothing ever appears broken"; in practice it hid a real outage behind a no-op, and a Buy button that returns you to the page you were on reads as an abandoned product.
- **NEW `/api/health` + `configIssues()` in `env.ts`** — turns silent degradation into something curl-able. 200 `{ok:true}` when the deployment can take money and recover an account; **503** with the offending variable NAMES + user-facing impact otherwise. Never returns a value, so it's safe to expose.
- **Escalating PIN lockout** (`lockoutSecondsFor`, +4 tests) — the flat 60s cool-down allowed ~300 guesses/hour, which walks a 4-digit PIN in under a day of unattended physical access. Now 60s → 2m → 4m → 8m → capped 15m (~20/hour), costing an honest fat-fingered cashier only the first minute.
- **Sitemap** now lists `/about` + all five legal routes (previously only `/`, `/sign-up`, `/sign-in`, so the policies Stripe expects to find were unlinked *and* unindexed). Dead `error` state removed from `/forgot-password` (the neutral confirmation is deliberate anti-enumeration — now documented as such).
- **Supersedes PR #170** (the free-trial copy fix) — its commit is included on this branch.

**Known-open after this branch:** per-item tax rates (one flat `Business.taxRateBps` can't serve mixed-taxability retail); Playwright still not in CI (2 smoke tests, run by hand); CSP still `Report-Only` (`CSP_ENFORCE = false`, deliberate hold pending installed-PWA verification); every route still `ƒ` dynamic because the root layout reads `headers()` for the nonce, so marketing/legal are `no-store` and un-CDN-cacheable; `NEXT_PUBLIC_APP_URL` still points at the `valla-pos.vercel.app` alias so Stripe/reset links take a 308 hop; desktop installer is Windows-only and unsigned while the pricing card says "Windows & macOS".

_Prior anchor: 2026-07-19 — newest: **Editions Stage 5b COMPLETE + the $99 license pipeline + the Rust boot-gate (#141–#156)** — the offline app now builds as a real static export (all 8 business pages converted to client-fetch through the seam, route-staging build, SQLite runtime boot, license gate), the $99 buy→sign→email→activate loop is built end-to-end in the main app, and the SQLite open is now gated on the compiled-Rust `check_license` Ed25519 anchor (#156), and native auto-print-on-sale is wired through the Rust `print_raw` (network path) with a printer config in Settings (#158); see the **"2026-07-19 — Editions Stage 5b done + $99 pipeline"** and **"2026-07-13 — Editions Stages 3–7"** sections. **996 tests green; cloud byte-for-byte unchanged.** **`tauri build` is GREEN — real installers now build** (verified 2026-07-19 on the Windows box: `vallapos.exe` + a WiX MSI + an NSIS setup.exe under `src-tauri/target/release/bundle/`, UNSIGNED). What's LEFT is only **signing certs + publish** (Stage 7b), the optional `serial` `print_raw` arm, and the **$99 go-live env** — turnkey steps in `docs/SHIP_DESKTOP.md`. Also: **"2026-07-19 — Payments PR-D: flat SaaS subscription"** below. Older history follows unchanged._
...
- **What:** new `CheckoutSession` model + `CheckoutSessionStatus` enum (`OPEN`/`CAPTURED`/`FAILED`/`EXPIRED`) + back-relations on `Business`/`Order`/`Payment`. It is the durable lifecycle anchor for async, processor-backed QR/card sales (Stripe hosted Checkout on the merchant's **connected account**).
- **Design call (resolved a scout disagreement):** record the sale with a `CheckoutSession` row and write the `Payment` row **only on webhook capture** — there is **never a `PENDING` Payment**. This structurally prevents Z-report/drawer inflation from unpaid attempts, and `stripeSessionId @unique` + `paymentId @unique` guard double-capture **at the DB layer** (Connect retries webhooks aggressively). `Payment.checkoutSession != null` distinguishes a processor-**verified** sale from the #72 operator-confirmed QR.
- **Key columns:** `stripeSessionId @unique` (webhook resolves tenant from THIS row, never from payload), denormalized `stripeAccountId` (webhook asserts `event.account ===` it), `@@unique([businessId, clientUuid])` (idempotent "Pay" re-tap), `amountCents`+`currency` (re-verified vs Stripe `amount_total` before PAID), nullable `paymentId @unique`/`expiresAt`.
- **Migration:** `20260719131746_payments_checkout_session` — **purely additive** (CREATE TYPE + CREATE TABLE + indexes/FKs; zero changes to existing tables). Generated offline via `prisma migrate diff` (schema-to-schema, no DB writes).
- **Verified:** `908 tests` green · typecheck · lint · `next build` all pass. Model is inert (nothing constructs it yet — PR-C wires it), so cash/confirm-QR/manual are byte-for-byte unchanged.
- **⚠ GATE (before merge):** apply the migration to Neon **from this branch** — `! npx prisma migrate deploy` (per [[vallapos-db-command-env]]: the gitignored `.env` holds `DATABASE_URL`+`DIRECT_URL`; running on `main` pre-merge finds nothing, and merging before applying 500s every authed request on the missing table). Then `prisma migrate status` clean → `gh pr merge`.

## 2026-07-19 — Editions Stage 5b done + the $99 license pipeline (offline app now builds + licenses; #141–#153)

The offline edition is now **feature-complete on the TS/build side** — it exports as a real static site, boots local SQLite in the Tauri webview, and gates on a signed license. All additive `*.local.tsx` variants swapped in only for the local build; **cloud byte-for-byte unchanged; 987 tests green.** What's left is purely the Rust/Tauri toolchain + certs (see the updated "Remaining" bullets in the 2026-07-13 section).

**The $99 license pipeline (#141–#144, MERGED)** — built in the **main app** (not a separate site), so the whole buy→sign→email→activate loop is one codebase. `src/features/desktop-license/*`: one-time Stripe Checkout (`mode:"payment"`) at `/desktop/buy`; webhook (`/api/desktop-license/webhook`, signature-verified) that on payment signs an Ed25519 license via `loadLicenseSigner(LICENSE_SIGNING_SK)` → shared `issueLicense`, persists a `License` row (idempotent on `stripeSessionId`), and delivers via Resend; `/desktop/license` download page. Marketing "$99 Buy" button → `/desktop/buy`. Pure `renderLicenseEmail` extracted so tests don't touch env.

**Static-export offline app (#145–#153, MERGED):**
- **#145 route-staging build** — `scripts/build-local.mjs` (EXCLUDE cloud-only routes + SWAP each cloud `page.tsx` for its `*.local.tsx` client variant; manifest restore + crash recovery + `--dry`); `cross-env` for Windows. **#146 Web Crypto PIN** — `sqlite/pin.ts` PBKDF2 (SubtleCrypto) replaces `node:crypto` scrypt so PINs hash in the webview; also store `createSimpleItem`/`deleteItem`.
- **#147–#150, #153 page conversions** — all 8 business pages as client-fetch (`getLocalStore()`, poll `isLocalStoreReady()`): register (cash checkout), products, orders, receipt (`?order=` query param — export can't pre-render dynamic `[orderId]`), reports, drawer, **staff**, **settings**; + root/group/business `layout.local.tsx` shells + local nav. Settings/staff added 5 additive `SqliteDataStore` methods (`get/updateBusinessSettings`, `addOperator`/`setOperatorPin`/`deactivateOperator`, `BusinessSettingsRow`) + tests.
- **#151 runtime boot** — `LocalStoreBootstrap` dynamic-imports `@tauri-apps/plugin-sql`, `Database.load("sqlite:vallapos.db")` → `createTauriSqlDriver` → `initLocalStore`. **#152 license gate** — `LocalLicenseGate` (verifies the stored license on boot via `webcryptoEd25519Verifier(LICENSE_PUBLIC_KEY)`, else a paste-key entry screen; `@tauri-apps/plugin-store` KV) + `src/lib/license/public-key.ts` (baked-in `88afc7ae…`, == `license.rs`). JS deps `@tauri-apps/plugin-sql` + `plugin-store` installed (pinned).
- **Verified end-to-end (2026-07-19):** signed a real test license with the production private key and confirmed it **verifies against the baked-in `public-key.ts`** — the exact check the gate runs — so the buy→sign→activate loop is proven in code (only the running `.exe` remains).

## 2026-07-13 — Editions Stages 3–7 (offline edition: data + auth + Tauri seam + license + release; #119–#125)

Executing the two-edition split (`docs/EDITIONS.md`; see [[vallapos-editions-direction]]). The offline (local) edition's `SqliteDataStore` — hand-written SQL behind the DataStore seam, no Prisma, for a small Tauri bundle — is now **feature-complete and fully tested** (via `node:sqlite`, zero new deps). All additive/inert: nothing constructs the store yet (needs the Tauri driver, Stage 5), so the **cloud app is unchanged**, the shared `DataStore` interface and the tenant CI guard untouched.

- **#119 Stage 3a (MERGED)** — schema (`sqlite/schema.ts`, cash subset), `SqlDriver` port, `migrate()` + `getRegisterCatalog`.
- **#120 Stage 3b (MERGED)** — order-history reads (`listOrders`, `getOrderReceipt`), tenant-scoped.
- **#121 Stage 3c (MERGED)** — the rest: `getManagedCatalog`; the reports (`getDailyReport` with the SAME reconciliation semantics as cloud — payment-time windows, status-agnostic movements incl. negative refund reversals, proportional refund back-out, verified/unverified tender split; `getItemSalesReport`; `getCashierSalesReport`); the four drawer reads; the **atomic `checkout`** (resolve lines from the local catalog → shared pure `computePricedOrder`/`validateGroupSelection` so money/tax is byte-for-byte identical to cloud → idempotent on `clientUuid` → allocate-order-number + insert Order/OrderLine[]/OrderLineModifier[]/Payment under `BEGIN IMMEDIATE`; SQLite single-writer, so no order-number race; drops the cloud-only manager-gate + offline snapshot); `openDrawer`/`closeDrawer` (shared `reconcile`); and local operator/PIN (`listOperators`, `verifyOperatorPin` reusing the scrypt `verifyPin`) + idempotent `seedFirstRun` (one business+operator, pinned `LOCAL_BUSINESS_ID`). Operator/PIN + seed are **SqliteDataStore-only** (NOT on the shared interface — cloud uses Better Auth+Membership). Also hardened schema timestamp DEFAULTs to ISO-8601 UTC. **830 tests green; typecheck + lint clean.**

- **#122 Stage 4 (MERGED)** — **local auth + env branch (runtime), cloud byte-for-byte unchanged** (all gated on `isLocal`, default cloud; 837 tests green incl. the full cloud env/tenant suites). `edition.ts` gained fixed ids `LOCAL_BUSINESS_ID`="local" + `LOCAL_USER_ID`="local-user" (pure module; `sqlite-store.ts` re-exports the former). `env.ts` branches: the cloud-required vars (`DATABASE_URL`, `BETTER_AUTH_SECRET`/`URL`, `NEXT_PUBLIC_APP_URL`) collapse to `.default()` placeholders in local (keeps inferred TYPE=`string` so cloud consumers compile+behave identically); added optional `VALLA_LOCAL_DEVICE_SECRET`; prod Upstash fail-fast gated `!isLocal`. `tenant.ts`: `authMode==="pin-only"` → `requireMembership`/`requireSession` return the fixed single-operator OWNER context WITHOUT calling `auth`/`db`. `operator.ts`: HMAC key from the device secret in local.

- **#123 Stage 5a (MERGED)** — Tauri seam wiring + shell scaffold (additive, dep-free, tested; cloud unchanged). `sqlite/tauri-driver.ts` (`createTauriSqlDriver` over a minimal `TauriSqlDatabase` shape — no `@tauri-apps` dep), `data-store/local.ts` (`createLocalDataStore` — migrate+seed, client-safe), `peripherals/transports/tauri.ts` (`createTauriPrinter` via injected `invoke`→`print_raw`), `peripherals/auto-print.ts` (getReceipt→fromOrderReceipt→formatReceipt→print chain). `src-tauri/` Tauri v2 Cargo scaffold (`print_raw` TCP-9100 + `open_drawer`) — **NOT cargo-build-verified** (no local Rust). `next.config.ts` gates `output:'export'` for local; cloud unchanged.
- **#124 Stage 6a (MERGED)** — Ed25519 license core: `src/lib/license/{license,webcrypto}.ts` (wire format `VLK1‖ver‖len‖payload‖sig64` Crockford-Base32, canonical-JSON payload, crypto-injected sign/verify; 13 tests) + `src-tauri/src/license.rs` (`ed25519-dalek verify_strict`, the real trust anchor; `check_license` cmd; zero-placeholder `PUBLIC_KEY`).
- **#125 Stage 7a (MERGED)** — `.github/workflows/desktop-release.yml` (tauri-action matrix win+mac, `v*`-tag/dispatch only, never gates PRs) + `docs/RELEASING.md` (signing matrix/costs, cut-a-release steps, no-phone-home revocation).
- **#126 Stage 6b-core (MERGED)** — license gate/issuance/store TS: `src/lib/license/{gate,store,issue}.ts` — `resolveLicenseState`→licensed|unlicensed|invalid(reason) (injected loader+verifier), `createLicenseStore(kv)` (Tauri store plugin→`$APPCONFIG/license.vlk`; empty⇒null), `issueLicense({sku,id,iat,ex?},sign)` (vallahub signer, holds no secret). +7 tests (issue→store→gate round-trip incl. tamper/expiry/revocation).
- **#138 Stage 5b — runtime glue (partial, MERGED)** — `data-store/local-runtime.ts`: a process-singleton (`initLocalStore(driver, seed?)` / `getLocalStore()` / `isLocalStoreReady()`) that boots `createLocalDataStore` ONCE per Tauri webview and exposes it synchronously to the (future) client page islands. Idempotent (concurrent/repeat calls collapse to one boot; resets on a failed boot so a later call can retry), **dep-free** (injected driver) + client-safe. Unit-tested via `node:sqlite` (+4). Consumed by nothing yet → cloud unchanged. This is the only part of 5b verifiable headlessly; the rest needs a Rust machine (below).

**812→987 tests green across the whole editions arc (through the #145–#153 5b page arc); cloud byte-for-byte unchanged throughout.** _See the "2026-07-19 — Editions Stage 5b done + $99 pipeline" section below for the page-conversion / runtime / license-gate / $99-pipeline detail — the bullets below are UPDATED to what still remains._

**Remaining (need the Rust/Tauri toolchain + certs + a running local app — NOT doable/verifiable in this headless env; turnkey steps in `docs/SHIP_DESKTOP.md`):**
- **Stage 5b-runtime (page conversions + SQLite boot ✅ #145–#153; only the native shell remains):** the `sql`+`store` plugins are ALREADY registered (verified 2026-07-19 — Cargo.lock sql 2.4.0/store 2.4.4, `.plugin(…)` in `src-tauri/src/lib.rs:101–102`, `sql:default`/`store:default` capabilities, JS `@tauri-apps/plugin-sql@2.4.0`+`plugin-store@2.4.4` pinned to the SAME versions → no JS↔Rust skew; `tauri icon` already run — icons present). **Native auto-print WIRED (#158):** the register calls `autoPrintOrder` after a committed cash sale → `getOrderReceipt` → shared escpos formatter → `createTauriPrinter` → Rust `print_raw` (best-effort, non-blocking); device-local printer config (`src/features/peripherals/printer-config.ts`, a Tauri KV) edited in Settings → "Receipt printer"; shared `lib/tauri/{runtime,store-kv}.ts` helpers extracted (+4 tests, 996 total). So what's left: `tauri build` (the `.exe`); finish the Rust `print_raw` **`serial`** arm — the **`windows_spooler` arm is implemented (#160, hand-written `winspool` Win32 FFI, RAW datatype) and `cargo build` is GREEN (verified on the Windows box 2026-07-19, cargo 1.97.1, MSVC — compiles + links, `winspool.lib` resolves; only a pre-existing unrelated dead-code warning in `license.rs`)** so network + Windows printers both print; offline receipts also print via the `/receipt` page.
- **Stage 6b-shell (JS gate + Rust boot-gate + $99 issuance ✅ #141–#144, #152, #156):** the license entry screen (`LocalLicenseGate` → `resolveLicenseState`/`createLicenseStore`, `$APPCONFIG` store) + baked-in `public-key.ts` are shipped and a signed test license verifies against them. **The Rust boot-gate is now wired (#156):** `src/lib/license/native.ts` bridges the webview to the Rust `check_license` anchor and `local-bootstrap.tsx` calls it BEFORE `Database.load` — the SQLite store opens ONLY on a valid Ed25519 signature verified in compiled Rust (JS gate = UX layer, Rust = authoritative at the data boundary); `tauri-kv.ts` shares the plugin-store KV between the two. The **$99 buy→sign→email→activate loop is built in the main app** (`src/features/desktop-license/*`: Stripe one-time Checkout `/desktop/buy`, webhook signs via `LICENSE_SIGNING_SK`, Resend delivery, `/desktop/license` download). What remains: the signed revocation blocklist (empty today); go-live env (`STRIPE_*`, `LICENSE_SIGNING_SK`, `RESEND_*`, `DESKTOP_DOWNLOAD_URL`) + apply the `License` migration to Neon + point the Stripe webhook at `/api/desktop-license/webhook`.
- **Stage 7b:** buy/configure signing certs (~$220–320/yr yr-1) + set Actions secrets; tag `vX.Y.Z`; publish the drafted Release; link installers from vallahub.com; ship the Test-print/Open-drawer diagnostic.
- Also long-standing: `#94/#95` open (No/Extra line modifiers); the payments PR-B/C/D sequence (gated on Stripe env + go).

## 2026-07-12 — DB connection pooling (PR #115, OPEN)

**Capacity fix.** Production `DATABASE_URL` was pointing at Neon's **direct, non-pooled** host (`ep-broad-frog-atukzrs2.c-9…`, no `-pooler`), so each serverless instance held its own direct Postgres connections — ceiling ≈ Neon `max_connections` (~100 on small compute ⇒ ~40–75 concurrent cashiers, `too many connections` under burst). **PR #115** adds `directUrl = env("DIRECT_URL")` to the datasource (runtime uses the pooled `DATABASE_URL` w/ `pgbouncer=true`; the Prisma CLI uses the direct `DIRECT_URL` for migrations, which can't run through transaction-mode pooling). `DIRECT_URL` is **optional** in `env.ts` (client never reads it → not needed in Vercel; `next build`/`prisma generate` don't evaluate it, so an unset value can't break the build). **No schema/data migration**; inert until the env var is swapped. **Operator steps (post-merge):** copy Neon's pooled string (`-pooler` host + `&pgbouncer=true`) → set `DATABASE_URL` pooled + add `DIRECT_URL` direct in local `.env`; swap `DATABASE_URL` (Prod+Preview) in Vercel + redeploy; verify with `npx tsx prisma/smoke-order-race.ts` (50 concurrent tx → `PASS`). Lifts the ceiling to several hundred concurrent cashiers; next ceiling becomes the Upstash free-tier 10k-commands/day cap.

## 2026-07-08 — Stripe Connect scaffold + hardware self-check + bulk catalog entry (#91–#93 merged; #94/#95 open)

Three PRs merged to `main` since the 2026-06-26 peripherals batch, plus two open feature PRs. **Suite ~586 tests green** (per commit messages — #91 branch 561, #92 550, #93 575; not re-run this session, `node_modules` absent). CI `quality` + Vercel deploy ran on each merge.

- **#91 (MERGED) — Stripe Connect onboarding scaffold (PR-A).** First slice of integrated payments, executing the **now-locked** payments direction in `docs/PAYMENTS.md §9` (multi-merchant **Stripe Connect, Accounts v2, SaaS/DIRECT charges — the business is merchant of record, VallaPOS takes NO cut**; country-agnostic hosted-Checkout QR rail across US+MX+BR; flat SaaS subscription as our revenue). This PR only lets a business **connect its own Stripe account** — nothing moves money yet (the sale rail is the later PR-C).
  - **Schema (migration `20260707205659_payments_connect_onboarding`, additive/defaulted):** `Business` gained `country String @default("US")` (US|MX|BR), `stripeAccountId String? @unique`, `stripeChargesEnabled Boolean @default(false)`. **Migration APPLIED to Neon** (the PR-A commit body was written before it was applied and says "NOT yet applied" — it has since been applied; verify with `prisma migrate status`).
  - **Architecture:** a `ConnectGateway` PORT + in-memory fake so onboarding orchestration is fully unit-tested; the real gateway (`connect-stripe.ts`) is the only module touching Stripe, via `fetch` against the pinned v2 API (`2026-06-24.dahlia`). `stripe` SDK pinned **22.3.0** (used for webhook signature verification only).
  - **Wiring:** `manage_settings`-gated server actions (start/resume onboarding — idempotent per business, live status refresh) + a cached-status query; webhook `app/api/payments/webhook` (signature-verified `account.updated` → flips the cached charges-enabled flag); Settings → **Payments** UI section (Beta) with Connect / Finish / Refresh and a **dormant notice when `STRIPE_*` env is unset**.
  - **DORMANT in prod:** all `STRIPE_*` env vars are optional and unset in Vercel, so the feature degrades to OFF and the build never throws (mirrors the Upstash/Resend lesson). No live payment path exists yet.
- **#92 (MERGED) — Hardware Readiness self-check (Settings → Devices, Beta, `manage_settings`-gated, hardware-free).** A page a merchant opens on their own phone/laptop to see in ~30s whether THIS browser+device can drive VallaPOS peripherals: a capability panel (secure-context + WebUSB/WebHID/Web Serial/Web Bluetooth feature detection, each with what it unlocks or the workaround) + a **barcode-scanner tester** (detects a real HID wedge scanner from manual typing by keystroke-burst timing) + a copy-diagnostics button. Rendered beside the existing Receipt-printer (Test-print preview) card. Client-only, no schema. (`src/features/peripherals/components/HardwareReadiness.tsx`.)
- **#93 (MERGED) — Bulk paste-or-type catalog entry.** Products → **"Bulk add"**: a spreadsheet-style grid you can type across (a blank row auto-appears) or **paste straight from Excel/Sheets/Notes**, with live per-row validation and one "Save all". Preset-aware from `Business.mode` (Menu · Retail adds a SKU/barcode column · Services default to SERVICE); one-cell multi-size prices (`"Small:2.50, Large:3.50"`); categories auto-created (case-insensitive reuse); SKU conflicts + invalid rows are **skipped + explained, not dropped**; all created in one transaction. Plus multi-line **modifier-group creation** ("Oat milk +0.75" per line). Pure `bulk-parse.ts` (US/EU/LATAM money formats) shared by the live preview + authoritative server re-validation; `bulkCreateItems` + `createModifierGroupWithModifiers` actions (`manage_products`-gated, tenant-scoped). **No schema change** (+25 tests).
- **#94 (OPEN) — per-item "No/Extra" ingredient options** (catalog side).
- **#95 (OPEN) — register cashier-typed "No/Extra" line modifiers with upcharge** (register side). Both pending a merge decision.

**Payments direction is now LOCKED** (`docs/PAYMENTS.md §9`, 2026-07-07): the §6 open decisions are resolved. Sequenced PRs behind `PAYMENTS_V2_ENABLED` (cash/manual path untouched): **PR-A ✅ (#91 above)** → **PR-B** Payment/PaymentIntent schema deltas (own branch, Neon before merge) → **PR-C** QR sale rail (Checkout Session on the connected account + webhook `OPEN`→`CAPTURED`) → **PR-D** flat SaaS subscription (platform Billing + Customer Portal + gate app access on status) → (later) native shell → Stripe Terminal / Tap to Pay. **PR-B/PR-C/PR-D still pending the user's Stripe env + go decision.**
...
- **#77 CSP enforce (R-5):** moved CSP from report-only to **ENFORCED with per-request nonces** via `middleware.ts` + pure `src/lib/csp.ts` (`script-src 'self' 'nonce-…' 'strict-dynamic'`, **no `unsafe-inline` on scripts**; styles keep `unsafe-inline` — inline style attrs can't be nonce'd; report endpoint kept). next-themes script gets the nonce. **Verified live on prod**: enforced header + nonce on all 25 inline scripts + the browser E2E passed under it.
  - **Correction 2026-07-21:** the above overstates the ship. Production actually serves `Content-Security-Policy-Report-Only` — `CSP_ENFORCE = false` in `middleware.ts` — so the nonce machinery is live but enforcement is NOT. Switching to the enforced header is still pending the live-device (installed-PWA) verification. See docs/audit/security.md.
...
**Suite: 550 tests green on `main`** (279 → +271; recent: #77 CSP +9, #78 manager-gate +20, peripherals groundwork #83/#84 +62, peripherals Phase 1 #86/#87/#88 +77). Plus the Playwright E2E harness (runs separately via `npm run test:e2e`, live-green against prod). CI `quality` is green again — Actions billing was resolved 2026-06-26 (verified passing in ~1m26s on `main`); Vercel deploys ran on each merge.
...
**Peripherals Phase 1 — built HARDWARE-FREE & merged (#86/#87/#88/#89).** The "we likely won't have a printer to test" problem was solved by validating everything that *can* be validated in software:
- **#87 virtual transport + `preview.ts`** — captures the ESC/POS bytes and renders a pure, deterministic **on-screen receipt preview** (decodes text/align/bold/size + QR/cut/drawer-kick markers). The test harness AND a real feature. +30 tests.
- **#86 WebUSB adapter (`transports/webusb.ts`)** — real `navigator.usb` transport (vendor-filtered picker → open/claim → chunked `transferOut` → `getDevices()` reconnect + connect/disconnect events). `usb` is **injectable → fully mock-tested**, SSR-safe; Windows `usbprint.sys` driver-claim caveat documented. +20 tests.
- **#88 Star CloudPRNT (`cloudprnt.ts` + `app/api/cloudprnt/[deviceToken]/route.ts` + `scripts/cloudprnt-sim.mjs`)** — printer-polls-server HTTP path (dodges HTTPS→HTTP mixed content), tenant-scoped by device token, software-validated end-to-end with a fake-printer simulator. **In-memory queue is NOT serverless-durable** → production follow-up = back the `QueueStore` with **Upstash** (recommended, no migration) or a Prisma `PrintJob` model. +27 tests.
- **#89 Settings → Devices screen (Beta, manage_settings-gated)** — connect a USB Epson/Star printer (one-time picker → auto-reconnect), **Test print renders the on-screen preview with NO hardware** + sends to a connected unit, Open-drawer kick, friendly errors (WebUSB-unsupported, the Windows driver-claim → Zadig hint). Does NOT touch the live checkout/money path.

**What now genuinely needs ONE real printer (and how it self-closes):** only the physical handshake — that `open/claim/transferOut` succeeds on a live Epson/Star unit, the paper feeds/cuts, the drawer pops, and the registry's product IDs match. Built defensively so the **first person with a printer** (you later, a beta merchant, a friend) confirms it in one "Test print" tap, and the registry auto-corrects the PID from the live descriptor. Deferred to Phase 2: wiring auto-print-on-cash-sale (behind `PERIPHERALS_V2`), the CloudPRNT durable store, Web Bluetooth, and the local-bridge/native-shell zero-touch path.
...
**Build-next (agent-doable):**
- **Payments direction is DECIDED** (`docs/PAYMENTS.md §9`) and **PR-A shipped (#91 Connect onboarding scaffold, dormant)**. Remaining sequenced work: **PR-B** (Payment/PaymentIntent schema deltas, own branch, Neon-before-merge) → **PR-C** (QR sale rail = Checkout Session on the connected account + `app/api/payments/webhook` `OPEN`→`CAPTURED`) → **PR-D** (flat SaaS subscription). **These are gated on the user setting the `STRIPE_*` env vars in Vercel + a go decision** — the rail stays dormant until then.
- **#94 / #95** (per-item + register-typed "No/Extra" line modifiers with upcharge) are **OPEN** and need a merge decision.
- **Migration-gated (own branch each):** image tiles + open-tickets/save-cart (register UX uplift remainder), a `Customer` model → customer/loyalty. Multi-sensory tap confirmation is partially in (press `active:scale-[0.98]` everywhere; haptic/sound not yet).
- CSP ships **report-only with per-request nonces (#77)** — `CSP_ENFORCE = false` in `middleware.ts`; enforcement is pending live-device (installed-PWA) verification (corrected 2026-07-21, see `docs/audit/security.md`).
