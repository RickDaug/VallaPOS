...
_Last updated: 2026-07-19 — newest: **Editions Stage 5b COMPLETE + the $99 license pipeline (#141–#153)** — the offline app now builds as a real static export (all 8 business pages converted to client-fetch through the seam, route-staging build, SQLite runtime boot, license gate) and the vallahub $99 buy→sign→email→activate loop is built end-to-end in the main app; see the **"2026-07-19 — Editions Stage 5b done + $99 pipeline"** and **"2026-07-13 — Editions Stages 3–7"** sections. **987 tests green; cloud byte-for-byte unchanged.** What's LEFT is only the Rust/Tauri toolchain + signing certs (register the `sql`/`store` plugins, `cargo build`/`tauri build`, native printer, signed release) — turnkey steps in `docs/SHIP_DESKTOP.md`. Also: **"2026-07-19 — Payments PR-D: flat SaaS subscription"** below. Older history follows unchanged._
...
- **Stage 6b-shell (JS gate + $99 issuance ✅ #141–#144, #152; only the Rust boot-gate remains):** the license entry screen (`LocalLicenseGate` → `resolveLicenseState`/`createLicenseStore`, `$APPCONFIG` store) + baked-in `public-key.ts` are shipped and a signed test license verifies against them. The **$99 buy→sign→email→activate loop is built in the main app** (`src/features/desktop-license/*`: Stripe one-time Checkout `/desktop/buy`, webhook signs via `LICENSE_SIGNING_SK`, Resend delivery, `/desktop/license` download). What remains: boot-gate the SQLite open on Rust `check_license` (`license.rs` `PUBLIC_KEY` already == `public-key.ts`); go-live env (`STRIPE_*`, `LICENSE_SIGNING_SK`, `RESEND_*`, `DESKTOP_DOWNLOAD_URL`) + apply the `License` migration to Neon + point the Stripe webhook at `/api/desktop-license/webhook`.
