...
_Last updated: 2026-07-13 — see the **"2026-07-13 — Editions Stages 3–4"** section just below for the newest work (offline SqliteDataStore complete + local auth/env branch; #119–#122 merged; Stage 5 = Tauri shell next). Older history follows unchanged._
...
## 2026-07-13 — Editions Stages 3–4 (offline SqliteDataStore complete + local auth/env branch; #119–#122)
...
**Known deferred (wired later):** operator ATTRIBUTION (`Order.cashierId` / drawer `openedById`) left null; **tree-shaking `auth`/`auth-client`/`redis`//api/auth OUT of the local build** + the client-side operator PIN lock — both need Stage 5's `output:'export'` + edition-gated imports (in local those modules just aren't *called*). **Next: Stage 5** — Tauri shell (`src-tauri/` Cargo: tauri, tauri-plugin-sql, escpos crate, ed25519-dalek) + native `print_raw`/`open_drawer` + `transports/tauri.ts` + `next.config` `output:'export'` for local + `index.ts` `isLocal`→`SqliteDataStore` wiring + caller rewire + client PIN lock. Then 6 (license gate + vallahub issuance), 7 (packaging/code-signing ~$220-320/yr). **Stage 5 creates a real Rust project + needs toolchain/packaging decisions — a good check-in point.**
