...
- Lower: R-5 CSP still report-only + no report-uri; R-7 offline PII unencrypted at rest (accept/document); R-9 tenant guard is heuristic. **R-10 (non-negative prices) already enforced** (zod `min(0)` on all price inputs). **R-11 DONE** — store checkout now stamps `Order.cashierId` (#51; tabs already did), surfaced as a "Sales by cashier" Z-report table (#52).
