...
**Suite: 347 tests green on `main`** (279 → +68; #62 added +8). CI `quality` still can't run (Actions billing suspended); Vercel deploys ran on each merge.

**Payments — decisions needed before a real integration (#58):** (1) QR rail + first market — Stripe Payment Links (US) vs regional (SPEI/CoDi MX, PIX BR, UPI IN); (2) Stripe Terminal vs Connect; (3) fee/monetization model; (4) native shell RN vs Capacitor (gates ALL card-present). Plus: ~~generic "Manual/Other" tender~~ (✅ shipped #62); PaymentIntent record for cash too or async-only? confirm cards never queue offline; webhook route (`app/api/payments/webhook`) + Stripe secret.
