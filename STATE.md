...
**812→871 tests green across the whole editions arc; cloud byte-for-byte unchanged throughout.**

**Remaining (need the Rust/Tauri toolchain + certs + a running local app — NOT doable/verifiable in this headless env):**
- **Stage 5b:** `cargo build` the shell; convert cash-path `page.tsx` shells to client-fetch through the seam (gated `isLocal`); wire register→`createLocalDataStore`+auto-print; `DevicesManager` transport swap; inject the real `@tauri-apps` `Database`/`invoke`; add the JS deps + `@tauri-apps/cli` (+lockfile) + `npx tauri icon`.
