...
- **Stage 5b-runtime (page conversions + SQLite boot ✅ #145–#153; only the native shell remains):** `cargo build` / `tauri build`; `npx tauri icon`; **register the `tauri-plugin-sql` + `tauri-plugin-store` plugins in `src-tauri`** (the JS side already calls them via `LocalStoreBootstrap`/`LocalLicenseGate`); wire register→auto-print + `DevicesManager` native transport (offline receipts currently print via the `/receipt` page). JS deps `@tauri-apps/plugin-sql` + `plugin-store` already installed (pinned, lockfiled).
